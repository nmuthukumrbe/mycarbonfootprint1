...
(function(b) {
	...
	a.prototype = {
		...
		initModalStuff : function() {
			...
			this.getModalContent().append(this.getModalHeader()).append(
					this.getModalBody()).append(this.getModalFooter());
			return this
		},
		createModal : function() {
			var c = b('<div class="modal fade" tabindex="-1"></div>');
		...
		formatStringContent : function(c) {
			// FIXEDBY: Samy :caused the new line in the html content to replace with <br />
		...
		updateTitle : function() {
			if (this.isRealized()) {
				var c = this.getTitle() !== null ? this
						.createDynamicContent(this.getTitle()) : this
						.getDefaultText();
				this.getModalHeader().find("." + this.getNamespace("title"))
						.html("").append(c)
			}
			return this
		},
		...
		updateMessage : function() {
			if (this.isRealized()) {
				...
				this.getModalBody().find("." + this.getNamespace("message"))
						.html("").append(c)
			}
			return this
		},
			...
			if (this.isRealized()) {
				if (this.getButtons().length === 0) {
					...
					this.getModalFooter().find(
							"." + this.getNamespace("footer")).html("").append(
		...
		createHeaderContent : function() {
			//Below code is customized to adapt the Admin LTE
			var c = b("<button aria-label='Close' data-dismiss='modal' class='close' type='button'><span aria-hidden='true'>×</span></button><h4 class='modal-title'>"+this.getTitle()+"</h4>");
			//c.addClass(this.getNamespace("header"));
			//c.append(this.createTitleContent());
			//c.append(this.createCloseButton());
			return c
		},
		...
		enhanceButton : function(c) {
			...
			c.toggleSpin = function(g) {
				...
				if (g) {
					d.hide();
					c.prepend(e.createButtonIcon(e.getSpinicon()).addClass(
							"icon-spin"))
				} else {
					d.show();
		...
		updateClosable : function() {
			if (this.isRealized()) {
				this.getModalHeader().find(
						"." + this.getNamespace("close-button")).toggle(
						this.isClosable())
			}
			return this
		},
		...
		handleModalEvents : function() {
			this.getModal().on("show.bs.modal", {
				...
				var c = d.data.dialog;
				typeof c.options.onshow === "function" && c.options.onshow(c);
			...
			this.getModal().on(
					...
					function(d) {
						var c = d.data.dialog;
						typeof c.options.onshown === "function"
								&& c.options.onshown(c);
						c.showPageScrollBar(true)
					});
			this.getModal().on("hide.bs.modal", {
				...
				var c = d.data.dialog;
				typeof c.options.onhide === "function" && c.options.onhide(c)
			});
			this.getModal().on(
					"hidden.bs.modal",
					{
					...
					function(d) {
						var c = d.data.dialog;
						typeof c.options.onhidden === "function"
								&& c.options.onhidden(c);
			...
			this.getModal().on(
					...
					function(c) {
						c.target === this && c.data.dialog.isClosable()
								&& c.data.dialog.close()
					});
			this.getModal().on(
					"keyup",
					...
					function(c) {
						c.which === 27 && c.data.dialog.isClosable()
								&& c.data.dialog.close()
					});
		...
		makeModalDraggable : function() {
			if (this.options.draggable) {
				this.getModalHeader().addClass(this.getNamespace("draggable"))
						.on("mousedown", {
							dialog : this
						}, function(e) {
							var d = e.data.dialog;
		...
		realize : function() {
			this.initModalStuff();
			this.getModal().addClass(a.NAMESPACE).addClass(this.getType())
					.addClass(this.getSize()).addClass(this.getCssClass());
	...
	a.newGuid = function() {
		return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,
				function(f) {
					var e = Math.random() * 16 | 0, d = f === "x" ? e
							: (e & 3 | 8);
	...
	a.alert = function() {
		...
		return new a({
			...
			onhide : function(e) {
				!e.getData("btnClicked") && e.isClosable()
						&& typeof e.getData("callback") === "function"
						&& e.getData("callback")(false)
			},
			buttons : [ {
				label : d.buttonLabel,
				action : function(e) {
